...
const createCertificate = async (req, res) => {
  ...
  let formattedCertificate = {
      ...
      instrumentRange: newCertificate.instrumentRangeLow + "-" + newCertificate.instrumentRangeHigh
  };

  for (let i = 0; i < Object.keys(newCertificate).length - 13; i++) {
      const readingKey = i.toString();
  ...
  try {
      ...
      if (!lastCertificate || lastCertificate.certificate_number === null) {
          ...
          newCertificateNumber = lastNumber + 1;
      }
      ...
      // console.log('kirtansssscscscscscscscscscscccscs');
      ...
      const outputPath = path.join(__dirname, 'pdfs', `${formattedCertificateNumber}.pdf`);
